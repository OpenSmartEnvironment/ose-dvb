...
exports.playItem = function(player, item, cb) {  // {{{2
  var best;  // Best scored pledge
  var list = [];  // List of all pledges
  var counter = O.counter();

  var req = {  // Pledge request
    channel: item.data.ident,
    mplex: item.data.media.mplex,
    mcast: player.mcast && player.mcast.identify(),
    ucast: player.playback.data.ucast,
  ...
  for (var i = 0; i < player.dvb.length; i++) {
    ...
    counter.inc();
    new Pledge(player, e, req, onOpen);
  }
