...
exports.config = function(name, val, deps) {  // {{{2
  ...
  media.map({
    ...
    map: function(entry, cb) {
      cb([entry.dval.title, entry.id], entry.dval.mplex);
    },
 ...
 * from `pool`.
 *
 * @param trans {Object} Transaction of "media" shard for channel entries
...
//      console.log('ADDING CHANNEL', mplex.id + '_' + ch[12]);
      ...
      trans.add('dvbChannel', {
        alias: mplex.dval.alias + '_' + ch[12],
        title: ch[0],
        number: ch[12],
        mplex: mplex.id,
        ip: ip,
